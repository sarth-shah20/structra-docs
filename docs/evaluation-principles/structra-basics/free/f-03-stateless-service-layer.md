...
- User identity on each request: JWT tokens (the token itself contains the user ID and roles, cryptographically signed so the server trusts it without looking anything up).
