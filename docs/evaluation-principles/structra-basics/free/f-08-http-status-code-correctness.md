...
- 201 Created — Used specifically when a new resource has been created (POST requests that create entities).
- 204 No Content — Success, but there's nothing to return (DELETE requests, or PUT with no response body).
...
- 400 Bad Request — The request is malformed (invalid JSON, missing required field, invalid parameter value).
...
- 409 Conflict — The request conflicts with existing state (e.g., trying to create a user with an email that already exists).
