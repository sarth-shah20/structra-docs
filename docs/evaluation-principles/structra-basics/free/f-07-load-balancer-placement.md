...
- Clients randomly connect to different servers, but there's no health checking, no intelligent routing, and no single point of entry.
...
- IP Hash: The same client IP always routes to the same server. Useful when you need session affinity (though you should prefer stateless design instead).
