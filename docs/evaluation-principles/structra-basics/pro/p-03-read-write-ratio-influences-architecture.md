...
show:
        ...
        Write buffer / message queue as the ingestion layer. Instead of writing directly to the database,
        incoming writes go to a fast queue (Kafka, SQS). Workers drain the queue and write to the database at a
        controlled rate.
        LSM-tree-based storage engines (used by Cassandra, RocksDB) that are optimized for write throughput
