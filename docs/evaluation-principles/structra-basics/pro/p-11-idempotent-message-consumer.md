...
- Setting a value: UPDATE users SET email_verified = true WHERE user_id = 123 — running this twice changes nothing after the first execution. Idempotent.
- Incrementing a counter: UPDATE accounts SET balance = balance - 10 WHERE user_id = 123 — running this twice deducts 20. Not idempotent. Fix by recording the specific transaction ID and checking for it first.
