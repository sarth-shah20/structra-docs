...
- Fire too often on harmless conditions. CPU hitting 82% briefly during a batch job at 3 AM is not an incident. Over time, teams learn to ignore these alerts — and then miss the alert that actually matters.
- Don't reflect user impact. 100 errors during a period of 100,000 requests (0.1% error rate) is very different from 100 errors during a period of 500 requests (20% error rate). The threshold fires identically for both.
- Don't communicate urgency. Is this problem going to burn through the error budget in an hour or in a week?
