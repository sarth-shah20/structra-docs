...
aggregations. But they are not built to be authoritative data stores:
      ...
      Durability: Search engines prioritize search performance over ACID guarantees. They can lose recently
      indexed data in certain failure scenarios.
      Transactional consistency: Elasticsearch doesn't support multi-document transactions. You can't
      atomically update multiple documents.
      Authoritative source: Search indexes are typically refreshed on a configurable interval (Elasticsearch
      defaults to 1 second), meaning they are inherently eventually consistent.
