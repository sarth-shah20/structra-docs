...
Audit Log Properties
      ...
      Immutable: Audit logs must not be modifiable after the fact. Write-once storage, or logs shipped
      immediately to an append-only store.
      Separate from application data: The audit log should not be in the same database as the data it's
      auditing — a compromised application database should not compromise the audit trail.
