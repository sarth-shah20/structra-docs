---
sidebar_position: 1
title: Structra Documentation
slug: /
---

# Structra Documentation

Structra helps teams review software architecture quality using rule-based evaluation principles.

Use this documentation to understand evaluation rules, tier differences, and expected production-readiness standards.

## Available Documentation

- [Getting Started](/getting-started)
- [Account and Identity](/account-and-identity)
- [Evaluation Principles](/evaluation-principles)
- [Structra Basics](/evaluation-principles/structra-basics)
- [Structra Basics - Basic Rules (F-01 to F-20)](/evaluation-principles/structra-basics/free)
- [Structra Basics - Pro](/evaluation-principles/structra-basics/pro)
- [Structra Basics - Enterprise](/evaluation-principles/structra-basics/enterprise)
- [Production System Design Principles](/evaluation-principles/production-system-principles)

## What Structra Evaluates

- System reliability and failure handling
- Scalability planning and bottleneck prevention
- Data integrity and consistency boundaries
- Security, observability, and operational readiness
- Architecture fit for team size and product stage

## Access Model

- All users can read documentation.
- Evaluation enforcement is tier-based.
- Higher tiers apply stricter rules and more advanced checks.
